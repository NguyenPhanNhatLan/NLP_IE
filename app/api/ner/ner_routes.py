...
@router.post("/ner/insert-all", status_code=status.HTTP_201_CREATED)
...
async def delete_by_ids_endpoint(file: UploadFile = File(...), db=Depends(get_db)):
    ...
    try:
        existing = await db.labeled_data.find(
            {"id": {"$in": unique_ids}},
            {"_id": 0, "id": 1}
        ).to_list(length=None)
        ...
        del_result = await db.labeled_data.delete_many({"id": {"$in": unique_ids}})

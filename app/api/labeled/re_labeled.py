...
COLLECTION_NAME = os.getenv("RE_LABELED_COLLECTION", "re_labeled")
...
@router.get("/re/get-all", status_code=status.HTTP_200_OK)
async def get_all_docs(db=Depends(get_db)):
    try:
        collection = db[COLLECTION_NAME]
        docs = await get_all_labeled_data(collection)
        for d in docs:
            if "_id" in d:
                d["_id"] = str(d["_id"])
